...
func _on_timer_timeout():
	...
	conveyor_list.shuffle()
	get_node(conveyor_list[0]).get_node("Spawn Point").add_child(instance)
	instance.scale.x = 0.003
	instance.scale.y = 0.017
	instance.speed = egg_speed
