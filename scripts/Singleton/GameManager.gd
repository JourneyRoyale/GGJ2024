...
func _process(delta):
	if(isPlaying):
		...
		if time_since_last_increase >= increase_interval:
			...
			print("Score multiplier raised! Current multiplier: ", player_score_increase_rate_multiplier)
...
func successful_joke_score_increase():
	...
	print("Successful Joke! Score increased by: ",50*(player_score_increase_rate_multiplier + spotlight_bonus),"[Spotlight] ", spotlight)
